...
func (p Profile) GetTweetById(id TweetID) (Tweet, error) {
	return p.GetTweetByIdAsUser(id, UserID(0))
}

func (p Profile) GetTweetByIdAsUser(id TweetID, u_id UserID) (Tweet, error) {
	db := p.DB

	q, bind_values := tweet_select_query(u_id)

	var t Tweet
	err := db.Get(&t, q+`where id = ?`, append(bind_values, id)...)
